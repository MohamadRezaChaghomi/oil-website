...
export function SettingsTable({ settings, groups }: SettingsTableProps) {
  ...
  const [formData, setFormData] = useState({ key: "", value: "", group: "general", description: "", isPublic: false });
  ...
  const handleCreate = () => {
    ...
    setFormData({ key: "", value: "", group: "general", description: "", isPublic: false });
    setIsModalOpen(true);
  };
  ...
  const handleSave = async () => {
    try {
      ...
      if (formData.value.startsWith("{") || formData.value.startsWith("[")) {
        try { parsedValue = JSON.parse(formData.value); } catch { /* keep string */ }
      }
      const payload = {
        key: formData.key,
        value: parsedValue,
  ...
  const confirmDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/admin/settings/${selectedId}`, { method: "DELETE" });
      if (res.ok) {
  ...
  return (
    <>
      <AdminPageHeader
        ...
        action={
          <Button onClick={handleCreate} className="gap-2 bg-orange-600 hover:bg-orange-700 text-white">
          ...
          return (
            <div key={group} className="mb-8 last:mb-0">
              <h2 className="text-xl font-semibold text-foreground mb-3 border-b border-border pb-2">
                {group === "general" ? "عمومی" : group === "contact" ? "اطلاعات تماس" : group === "seo" ? "SEO" : group}
              </h2>
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-border">
                  <thead className="bg-muted/30">
                    <tr>
                      <th className="px-4 py-2 text-right text-sm font-medium text-foreground">کلید</th>
                      <th className="px-4 py-2 text-right text-sm font-medium text-foreground">مقدار</th>
                      <th className="px-4 py-2 text-right text-sm font-medium text-foreground">توضیحات</th>
                      <th className="px-4 py-2 text-right text-sm font-medium text-foreground">عمومی</th>
                      <th className="px-4 py-2 text-right text-sm font-medium text-foreground">عملیات</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-border">
                    {groupSettings.map((setting) => (
                      <tr key={setting._id} className="hover:bg-accent/50">
                        <td className="px-4 py-2 text-sm font-mono text-foreground">{setting.key}</td>
                        ...
                        <td className="px-4 py-2 text-sm text-muted-foreground">{setting.description || "-"}</td>
                        ...
                        <td className="px-4 py-2 text-sm">
                          <div className="flex gap-2">
                            ...
                            <button
                              ...
                              disabled={deletingId === setting._id}
      ...
      <Modal
        ...
        showConfirm={true}
      >
        {/* Custom content for modal - we need to override the default message */}
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-foreground mb-1">کلید *</label>
            <input
              type="text"
              value={formData.key}
          ...
          <div>
            <label className="block text-sm font-medium text-foreground mb-1">مقدار *</label>
            <textarea
          ...
          <div>
            <label className="block text-sm font-medium text-foreground mb-1">گروه</label>
            <select
            ...
            >
              ...
              <option value="contact">اطلاعات تماس</option>
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <label htmlFor="isPublic" className="text-sm text-foreground">قابل نمایش در API عمومی</label>
          </div>
        </div>
      </Modal>
